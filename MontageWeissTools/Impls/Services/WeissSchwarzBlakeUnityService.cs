...
public class WeissSchwarzBlakeUnityService
{
    ...
    private string GetExportDeckDataViaRegistry()
    {
        ...
        return System.Text.ASCIIEncoding.ASCII.GetString(blakeKey.GetValue(deckImportKey) as byte[]);
    }
    ...
    private DateTime? GetExportDateViaRegistry()
    {
        ...
        if (string.IsNullOrWhiteSpace(deckImportKey)) return null;
        var dateTimeString = System.Text.ASCIIEncoding.ASCII.GetString(blakeKey.GetValue(deckImportKey) as byte[]);
        // 16:32  11/14/2022\0
